...
fin = open(os.path.join('utils', 'mapping.pair'),'r')
replacements = []
...
def normalize(text, clean_value=True):
    ...
    if clean_value:
        ...
        # normalize postcode
        ms = re.findall(r'([a-z]{1}[\. ]?[a-z]{1}[\. ]?\d{1,2}[, ]+\d{1}[\. ]?[a-z]{1}[\. ]?[a-z]{1}|[a-z]{2}\d{2}[a-z]{2})',
                        text)
        if ms:
            sidx = 0
            for m in ms:
...
def loadData(mwz_ver, data_path, target_path):
    ...
    if not os.path.exists(target_path):
        ...
        with ZipFile(data_path, 'r') as zip_ref:
            ...
            shutil.copy(os.path.join(raw_path, 'ontology.json'), target_path)
            if(mwz_ver=="2.1"):
...
def createData(mwz_ver, target_path):
    # create dictionary of delexicalied values that then we will search against, order matters here!
    # dic = delexicalize.prepareSlotValuesIndependent()
    delex_data = {}

    fin1 = open(os.path.join(target_path, 'data.json'), 'r')
    data = json.load(fin1)
    if(mwz_ver=="2.2"):
...
def divideData(data, target_path):
    ...
    testListFile = []
    fin = open(os.path.join(target_path,'testListFile.json'), 'r')
    for line in fin:
        testListFile.append(line[:-1])
    fin.close()

    valListFile = []
    fin = open(os.path.join(target_path,'valListFile.json'), 'r')
    for line in fin:
...
def main(args):
    ...
    delex_data = createData(mwz_ver, target_path)
    print('Divide dialogues...')
    divideData(delex_data, target_path)


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    #parser.add_argument("--main_dir", type=str, default='data/mwz24')
    #parser.add_argument("--mwz_ver", type=str, default='2.4')
    #parser.add_argument("--target_path", type=str, default='data/mwz2.4')
