...
def get_dial(dialogue):
    ...
    dial = []
    d_orig = analyze_dialogue(dialogue, MAX_LENGTH)  # max turn len is 100 words
    if d_orig is None:
        return None
...
def loadData(mwz_ver, data_path, target_path):
    print(f"mwz_ver: {mwz_ver}")
    ...
    if not os.path.exists(target_path):
        ...
        with ZipFile(data_path, 'r') as zip_ref:
            ...
            raw_path = os.path.join(target_path, f"MultiWOZ_{mwz_ver}")
            zip_ref.close()
            shutil.copy(os.path.join(raw_path, 'data.json'), target_path)
            shutil.copy(os.path.join(raw_path, 'valListFile.json'), target_path)
            shutil.copy(os.path.join(raw_path, 'testListFile.json'), target_path)
            if(mwz_ver=="2.1"):
                shutil.copy(os.path.join(raw_path, 'system_acts.json'), target_path)
            else:
                shutil.copy(os.path.join(raw_path, 'dialogue_acts.json'), target_path)
            shutil.rmtree(raw_path, ignore_errors=True)

def fixMetaData(data):
    for dialogue_name in data:
        for idx, turn in enumerate(data[dialogue_name]['log']):
            metadata = turn['metadata']
            for domain in metadata:
                if "book" in metadata[domain]:
                    for slot in metadata[domain]["book"]:
                        if(slot=="booked"):
                            for j in range(len(metadata[domain]["book"]["booked"])):
                                for sk in metadata[domain]["book"]["booked"][j]:
                                    tmp_val = metadata[domain]["book"]["booked"][j][sk]
                                    if isinstance(tmp_val, list):
                                        val = tmp_val[0] if(len(tmp_val)>0) else ""
                                    else:
                                        val = tmp_val
                                    metadata[domain]["book"]["booked"][j][sk] = val
                        else:
                            tmp_val = metadata[domain]["book"][slot]
                            val = ""
                            if isinstance(tmp_val, list):
                                if(len(tmp_val)>0):
                                    val = tmp_val[-1]
                                ...
                                val = tmp_val
                            metadata[domain]["book"][slot] = val
                            
                if "semi" in metadata[domain]:
                    for slot in metadata[domain]["semi"]:
                        tmp_val = metadata[domain]["semi"][slot]
                        val = ""
                        if isinstance(tmp_val, list):
                            if(len(tmp_val)>0):
                                val = tmp_val[-1]
                            ...
                            val = tmp_val
                        metadata[domain]["semi"][slot] = val
            data[dialogue_name]['log'][idx]['metadata'] = metadata
    return data
    
def createDelexData(mwz_ver, target_path):
    ...
    data_file = os.path.join(target_path, 'data.json')
    with open(data_file) as fin1:
        data = json.load(fin1)
    if(mwz_ver=="2.2"):
        data = fixMetaData(data)

    if(mwz_ver=="2.1"):
        dialogue_act_file = os.path.join(target_path, 'system_acts.json')
    else:
        dialogue_act_file = os.path.join(target_path, 'dialogue_acts.json')
    with open(dialogue_act_file) as fin2:
    ...
    delex_file = os.path.join(target_path, 'delex.json')
    with open(delex_file, 'w') as outfile:
        json.dump(delex_data, outfile)

    return delex_data


def divideData(data, target_path):
    """Given test and validation sets, divide
    the data for three different sets"""
    testListFile = []
    with open(os.path.join(target_path, 'testListFile.json')) as fin:
        for line in fin:
            testListFile.append(line[:-1])

    valListFile = []
    with open(os.path.join(target_path, 'valListFile.json')) as fin:
        for line in fin:
            valListFile.append(line[:-1])

    trainListFile = open(os.path.join(target_path, 'trainListFile.json'), 'w')
    ...
    for dialogue_name in tqdm(data):
        ...
        if dial:
            ...
            dialogue['bs'] = []
            for turn in dial:
                dialogue['usr'].append(turn[0].strip())
                dialogue['sys'].append(turn[1].strip())
    ...
    trainListFile.close()
    # save all dialogues
    dev_path = os.path.join(target_path, 'dev_dials_delex.json')
    with open(dev_path, 'w') as f:
        json.dump(val_dials, f, indent=4)

    test_path = os.path.join(target_path, 'test_dials_delex.json')
    with open(test_path, 'w') as f:
        json.dump(test_dials, f, indent=4)

    train_path = os.path.join(target_path, 'train_dials_delex.json')
...
def buildDictionaries(word_freqs_usr, word_freqs_sys, target_path):
    ...
    with open(os.path.join(target_path, 'input_lang.index2word.json'), 'w') as f:
        json.dump(idx2words[0], f)
    with open(os.path.join(target_path, 'input_lang.word2index.json'), 'w') as f:
        json.dump(dicts[0], f)
    with open(os.path.join(target_path, 'output_lang.index2word.json'), 'w') as f:
        json.dump(idx2words[1], f)
    with open(os.path.join(target_path, 'output_lang.word2index.json'), 'w') as f:
        json.dump(dicts[1], f)


def main(args):
    mwz_ver = args.mwz_ver
    data_path = os.path.join("mwz_raw_data", f"MultiWOZ_{mwz_ver}.zip")
    target_path = f"mwz{mwz_ver}"
    loadData(mwz_ver, data_path, target_path)    
    print('Create delexicalized dialogues. Get yourself a coffee, this might take a while.')
    delex_data = createDelexData(mwz_ver, target_path)
    print('Divide dialogues for separate bits - usr, sys, db, bs')
    word_freqs_usr, word_freqs_sys = divideData(delex_data, target_path)
    print('Building dictionaries')
    buildDictionaries(word_freqs_usr, word_freqs_sys, target_path)

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--mwz_ver", type=str, default='2.4', choices=['2.0', '2.1', '2.2', '2.4'])
