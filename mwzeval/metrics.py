...
from mwzeval.normalization import normalize_data, normalize_state_slot_value
...
from mwzeval.utils import load_goals, load_booked_domains, load_belief_state, load_system_utterances
import copy
...
class Evaluator:
    ...
    def __init__(self, mwz_ver : str, mode : str, 
                 bleu : bool, success : bool, richness : bool, dst : bool, fuzzy_ratio : int):
        ...
        if bleu:
            self.reference_dialogs = load_system_utterances(self.mwz_ver, self.mode)
        ...
        if dst:
            self.gold_states = load_belief_state(self.mwz_ver, self.mode)
...
def get_bleu(input_data, reference_dialogs):
    ...
    for dialog_id, dialog in input_data.items():
        for turn_idx in range(len(dialog)):
            ...
            refs.append(reference_dialogs[dialog_id][turn_idx])    

    return {'bleu': corpus_bleu(hyps, [refs]).score}
...
def get_dst(input_data, reference_states, fuzzy_ratio, mwz_ver):
        ...
        for domain, state in state_dict.items():
            for s, v in state.items():
                if s in slot_detail:
                    ...
                    slot = s
                s_set.add(slot)
                val = normalize_state_slot_value(slot, v)
                constraints[(domain, slot)] = val
        return constraints, s_set
    ...
    if not has_state_predictions(input_data):
        ...
        num_turns = 0
        for dialog_id in input_data:
            if(dialog_id in reference_states):
                for i, turn in enumerate(input_data[dialog_id]):
                    ref, s_set_r = flatten(reference_states[dialog_id][i])
                    hyp, s_set_h = flatten(turn['state'])
                    
                    s_set_ref = s_set_ref.union(s_set_r)
                    s_set_hyp = s_set_hyp.union(s_set_h)
        ...
        joint_acc = joint_match / (num_turns + 1e-10) * 100
        slot_mismatch = s_set_hyp.difference(s_set_ref)
    ...
    return {
        ...
        'slot mismatch'   : list(slot_mismatch)
    }
